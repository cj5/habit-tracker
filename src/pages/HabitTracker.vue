...
import TableRow from '../components/weekly-habits/TableRow.vue';
import DatabaseTesting from '../components/weekly-habits/DatabaseTesting.vue';
...
function prevWeek() {
  const prevSunId = store.displayedDates[0].id - DAYS_TO_DISPLAY;
  updateDisplayedDatesArray(store.dates[prevSunId].title);
}

function nextWeek() {
  const nextSunId = store.displayedDates[0].id + DAYS_TO_DISPLAY;
